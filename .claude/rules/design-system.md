...
## Active Style: Soft Neutral (カスタムブランドカラー適用)

ベース: Soft Neutral スタイル。ブランドカラーで上書き済み。

- **Primary（メインカラー）**: #539d84（ブランドグリーン）→ ボタン、CTA、リンク、アクティブ状態
- **Accent（アクセント）**: #693c85（ブランドパープル）→ バッジ、ハイライト、ポイント装飾
- **Background**: 温かみのあるクリーム系オフホワイト
- **Card**: ホワイト + shadow-sm（ホバー時 shadow-md）
- **Radius**: rounded-xl（0.75rem）
- **Shadow**: shadow-sm on cards, hover:shadow-md

### デザインルール
1. カード: 白背景 + shadow-sm、ホバー時 transition-shadow + shadow-md
2. ボタン（Primary）: ブランドグリーン背景、白テキスト
3. アクセント装飾: ブランドパープルをバッジや下線に使用
4. 見出し: font-semibold、テキストカラーは foreground
5. 余白: p-6 または p-8 ベース（8pxグリッド厳守）
6. アニメーション: fade-in duration-200 でページ表示

## Active Layout: Landing Portfolio

構成: トップナビ + ヒーロー + 商品グリッド（3列）+ ブランドストーリー + SNS + 問い合わせフォーム
→ 1ページ縦スクロール型。セクションごとに `id` アンカーを設定。
